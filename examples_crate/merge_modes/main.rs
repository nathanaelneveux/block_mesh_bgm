...
use block_mesh_bgm::{
    binary_greedy_quads_with_config, BinaryGreedyQuadsBuffer, BinaryGreedyQuadsConfig,
};
...
fn main() {
    App::new()
        ...
        .add_systems(Update, toggle_merge_modes)
        .run();
}
...
fn setup(
    ...
    spawn_overlay(&mut commands, sample_mode, mesh_mode, config, stats);
...
fn toggle_merge_modes(
    keyboard: Res<ButtonInput<KeyCode>>,
    mut scene: ResMut<MergeModeScene>,
    mut meshes: ResMut<Assets<Mesh>>,
    ...
    let (mesh, stats) = build_mesh(&scene.samples, config, mesh_mode);
    *meshes
    ...
    overlay_text.single_mut().expect("overlay should exist").0 =
        overlay_string(sample_mode, mesh_mode, config, stats);
...
fn spawn_overlay(
    ...
    commands.spawn((
        Text::new(overlay_string(sample_mode, mesh_mode, config, stats)),
        Node {
