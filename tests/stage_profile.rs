...
use block_mesh_bgm::{
    binary_greedy_quads_profile, BinaryGreedyQuadsBuffer, BinaryGreedyStageTimings,
};
...
#[test]
#[ignore = "diagnostic stage-timing report"]
fn prints_binary_greedy_stage_breakdown_for_benchmark_cases() {
    let faces = RIGHT_HANDED_Y_UP_CONFIG.faces;
    let mut buffer = BinaryGreedyQuadsBuffer::new();

    println!("single-chunk cases:");
    for case in benchmark_cases() {
        let report = profile_case(&case, &faces, &mut buffer, 256);
        println!("{report}");
    }

    println!("multi-chunk cases:");
    for case in benchmark_multi_chunk_cases() {
        let report = profile_multi_case(&case, &faces, &mut buffer, 64);
        println!("{report}");
    }
}
...
fn profile_case(
    ...
    buffer: &mut BinaryGreedyQuadsBuffer,
    iterations: usize,
) -> String {
    let mut totals = BinaryGreedyStageTimings::default();
    let mut wall = Duration::ZERO;

    for _ in 0..iterations {
        let mut timings = BinaryGreedyStageTimings::default();
        let start = Instant::now();
        binary_greedy_quads_profile(
            &case.voxels,
            &case.shape,
            case.min,
            case.max,
        ...
        wall += start.elapsed();
        totals.column_build += timings.column_build;
...
fn format_stage_report(
    name: &str,
    iterations: usize,
    wall: Duration,
    totals: BinaryGreedyStageTimings,
) -> String {
    let stage_total = totals.total();
    let avg_wall_ns = wall.as_nanos() / iterations as u128;
    let avg_columns_ns = totals.column_build.as_nanos() / iterations as u128;
    ...
    format!(
        ...
        avg_visible_ns as f64 * 100.0 / avg_stage_ns as f64,
        avg_unit_ns as f64 * 100.0 / avg_stage_ns as f64,
        avg_carry_ns as f64 * 100.0 / avg_stage_ns as f64,
        avg_columns_ns as f64 / 1_000.0,
...
fn hash3(x: u32, y: u32, z: u32, seed: u32) -> u32 {
    ...
    value ^= y.wrapping_mul(0x85EB_CA77);
    value = value.rotate_left(13);
    value ^= z.wrapping_mul(0xC2B2_AE3D);
    value = value.rotate_left(17);
    value ^ seed.wrapping_mul(0x27D4_EB2F)
}
